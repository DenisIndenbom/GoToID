...
router.post('/my/edit', auth_handler('/login'), async function (req, res, next) {
	...
	const params = `&email=${new_email}&telegram=${new_telegram}&avatarURL=${encodeURIComponent(
		new_avatarURL
	)}`;
	...
	if (
		...
		!validate.name(new_lastName) ||
		!validate.name(new_firstName) ||
	...
	if (username !== new_username || firstName !== new_firstName || lastName !== new_lastName) {
		try {
			...
			if (e.code === 'P2002') return res.redirect(`/profile/my/edit?success=false${params}&unique=false`);
			else return res.redirect(`/profile/my/edit?success=false`);
		}
	...
	try {
		...
		// redirect to the edit page if the user is a P2002
		if (e.code === 'P2002') return res.redirect(`/profile/my/edit?success=false${params}&unique_tg=false`);
		else return res.redirect(`/profile/my/edit?success=false`);
	}

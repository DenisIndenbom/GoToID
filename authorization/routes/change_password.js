...
router.post('/', auth_handler('/login'), async (req, res) => {
	...
	if (!methods.validate.password(new_password))
		return res.redirect(`/change_password?success=false&bad_password=true`);
	...
	if (!user || !correct_password)
		return res.redirect(
			`/change_password?success=false&old_password=${old_password}${
				!correct_password ? '&wrong_password=true' : ''
			}`
		);
	...
	if (!update_user)
		return res.redirect(
			`/change_password?success=false&old_password=${old_password}&change_password_error=true`
		);

	...
	tgbot.onText(/\/gen_invite_code/, async (msg) => {
		...
		try {
			const invite_code = await prisma.InviteCode.create({
				data: {
					code: token.generate().slice(0, 6),
					account_type: args[2] == 'student' || args[2] == 'teacher' ? 'teacher' : 'student',
					expiresAt: new Date(Date.parse(args[1])),
				},
			});

			tgbot.sendMessage(msg.chat.id, 'Invite code: ' + '`' + invite_code.code + '`', {
				parse_mode: 'Markdown',
			});
		} catch (e) {
			tgbot.sendMessage(msg.chat.id, 'Incorrect args!', { parse_mode: 'Markdown' });
			tgbot.sendMessage(msg.chat.id, '`/gen_invite_code <expiresAt> <account_type>`', {
				parse_mode: 'Markdown',
			});
		}
	});
